...
### ステップ0: SPECファイルを作成（必須）

**⚠️ 重要: コンテンツ追加前に、必ずSPECファイルを作成してください。**

SPECファイルは、何を作るか、どんな作業をしたかを記録するドキュメントです。
後から「どうやって作ったっけ？」を振り返るために必要です。
...
**例:**
- `docs/specs/SPEC-auto-mode.md` - Auto Mode体験コンテンツの仕様
- `docs/specs/SPEC-notification.md` - 通知設定コンテンツの仕様
- `docs/specs/SPEC-plan-mode.md` - Plan modeカスタマイズの仕様
...
## 📋 概要
- **対象コンテンツ**: コンテンツ08: Auto Mode体験
- **作成日**: 2026-03-10
- **参照記事**: https://example.com/article
- **所要時間**: 20分
- **難易度**: ⭐⭐（中級）

## 🎯 学習目標
1. Auto Modeの基本的な使い方を理解する
2. 手動承認モードとの違いを理解する
3. シーンに応じた使い分けができるようになる
...
## 🎨 UI/UX設計
- メインカラー: 紫系（#667eea → #764ba2）
- 比較ボックス: 青系（#E1F5FE）
- Tipボックス: 黄色系（#FFF9C4）

## 🔧 技術実装
- ページファイル: `pages/08_🤖_Auto Mode体験.py`
- グラデーションヘッダー
- 3カラムレイアウト
- 比較表（手動 vs Auto）
- Tipボックス
...
**なぜSPECファイルが必要？**
1. **作業の記録**: 何を作ったか、どう作ったかが残る
2. **再現性**: 同じようなコンテンツを作る時の参考になる
3. **振り返り**: 後から「このコンテンツどうだったっけ？」を確認できる
4. **チーム共有**: 他の人が見ても理解できる
...
### 準備（最初に必ず実施）
- [ ] **📋 SPECファイルを作成した（`docs/specs/SPEC-<機能名>.md`）**
- [ ] 💰 コスト確認チェックリストを実施した（KNOWLEDGE.md参照）
- [ ] 💰 ハッカソン予算（3万円）の残額を確認した
...
- [ ] 💰 実装後のコスト確認（累計コストを記録）
- [ ] GitHubにpushした（ページ + テストファイル + SPECファイル）
- [ ] 本番環境で動作確認した
